...
function Feed() {
  ...
  // إذا لم يكن هناك مستخدم (نادراً ما يحدث داخل هذا المكون) نعرض محمل
              ...
              <div className="post-header">
                <Link to={`/profile/${post.user_id}`} style={{display:'flex',alignItems:'center',gap:'10px',textDecoration:'none',color:'inherit',flex:1}}>
                  <div className="post-avatar">
                    <div className="post-avatar-inner">{post.full_name ? firstLetter(post.full_name) : '؟'}</div>
                  </div>
...
function App() {
  ...
  const navigate = useNavigate(); // ملاحظة: useNavigate يجب استخدامه داخل Router، لذا سننقله إلى داخل component
  ...
  const handleLogout = () => {
    localStorage.removeItem('token');
    setUser(null);
    // استخدم navigate إذا كان متاحاً، لكننا سنعتمد على إعادة التوجيه التلقائي
  };
